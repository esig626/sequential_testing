...
Date: 16 September 2026. Updated: 24 September 2026. Local derivations are supported by working proofs and finite numerical checks where stated, not by default by independent review or a novelty determination.
...
| C31 | The exact Rényi chain rule iterates across general dependent transition kernels using tilted prefix laws; ordinary tensorisation is only a special product case | ESTABLISHED by iteration. The earlier Lei Yu monograph theorem citation was not freshly verified in the 24 September audit; the checked Polyanskiy and Wu source remains the foundation. |
...
| C46 | A minimising Renyi projection does not directly upper bound the selected singleton contribution, but a Holder change of measure through a tractable reference law gives an upper bound with a Renyi mismatch penalty; using the unselected root tilt bounds that penalty by minus log tilted branch reach | DERIVED, NOT INDEPENDENTLY REVIEWED as a project application; R048/S15 identify established risk sensitive variational prior art for the general comparison method. The reference moment still needs control. R044. |
| C47 | Finite length Markov exponential moment and simple testing bounds with explicit boundary terms are available in the literature | ESTABLISHED SOURCE RESULT, R045/S05. The selected initial state matrix formula in the report is an algebraic translation under a sufficient observed state assumption. |
| C48 | Adaptive conditional Rényi costs can be controlled through a cumulative path budget, including a fixed pair and suitable stopping | ESTABLISHED SOURCE RESULT, R046/S12. No such budget is asserted for arbitrary project kernels. |
| C49 | Positive operator and multiplicative regularity methods control dependent exponential functionals under explicit transform and integrability conditions | ESTABLISHED SOURCE METHODS, R047/S08/S09. Not an all orders consequence of ordinary mixing. |
| C50 | Exact variational reference infima for exponential functionals use both a Rényi penalty and a reference exponential moment | ESTABLISHED SOURCE RESULT, R048/S15. Account for the source normalisation and bounded function theorem before extending to unbounded conditional divergence. |
| C51 | Hidden Markov rate theorems, endpoint contraction and particle approximations each require separate translation to a finite retained record upper bound | VERIFIED SCOPE DISTINCTIONS, R049/R050/R051 and F026 to F028. They are not interchangeable information bounds. |
| C52 | Dependent finite sample concentration and Hellinger moment comparisons are established prior art relevant to our Hölder method | ESTABLISHED SOURCE RESULT, R052/S14. A joint versus product reference comparison is not automatically the arbitrary testing pair comparison. |
...
The 24 September literature audit is recorded in `sources/non_iid_renyi/REVIEW.md`, with 30 references and individual inspection depths. It did not rerun the historical numerical checks, modify production code, or independently review every source proof. C47 to C52 are literature statements and applicability distinctions, not new project theorems. The longer conversation report includes further exposition. No exhaustive novelty claim or sufficient sample complexity is asserted.
