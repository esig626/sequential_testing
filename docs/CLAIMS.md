...
Validation for the selected converse task: 47 passing local test methods, comprising the 30 existing methods and 17 new methods in `tests/test_selected_converse.py`. These are finite deterministic consistency checks, not an independent proof review, a novelty audit, a general numerical guarantee or a remote CI run. See `results/2026-09-16_selected_converse_checks.json` and `notes/2026-09-16_selected_renyi_converse_sample_complexity.md` for exact scopes.
